function distFromAverage(tab) {
 let avg = tab.reduce(function (a,b) {return a + b})/tab.length;
 console.log(avg);
 const diff = tab.map(function (e) {
   return Math.abs(e - avg);
 });
 console.log(diff);
}
...
function getEvenAverage(tab) {
 const even = tab.filter(function (el) {
   return el % 2 === 0;
 });
 ...
 return even.reduce(function (a,b){return a+b})/even.length;

 // return !even.length ? null : even.reduce(function (a,b){return a+b})/even.length;
}
